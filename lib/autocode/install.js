let autocode, colors, fs, install, mkdirp, path, request, semver, untar, userHome, zlib;
...
install = opts => {
	let access_token_url, config_resp, config_url, headers, host, j, len, module_is_lastest, module_name, module_parse, module_path, module_version, project, release, release_i, release_resp, release_url, release_version, releases, tag_name, tarball, tarball_response, tarball_url;
	host = 'github.com';
	...
	untar.untar(tarball).forEach(file => {
		let buffer, file_path, filename, i, link_path;
